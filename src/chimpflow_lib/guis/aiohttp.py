...
# Object managing chimpflow_composers.
from chimpflow_lib.composers.composers import (
    Composers,
    chimpflow_composers_get_default,
    chimpflow_composers_has_default,
    chimpflow_composers_set_default,
)

# Gui protocolj things (must agree with javascript).
from chimpflow_lib.guis.constants import Commands, Cookies, Keywords

logger = logging.getLogger(__name__)

thing_type = "chimpflow_lib.chimpflow_guis.aiohttp"


# ------------------------------------------------------------------------------------------
class Aiohttp(Thing, BaseAiohttp):
    """
    Object implementing remote procedure calls for chimpflow_gui methods.
    """
    ...
    async def activate_coro(self):
        ...
        try:
            ...
            if not chimpflow_composers_has_default():
                # The chimpflow_composer to use.
                chimpflow_composer_specification = {
                    "type": "chimpflow_lib.chimpflow_composers.html"
                }

                # Set up the default chimpflow_composer.
                chimpflow_composer = Composers().build_object(
                    chimpflow_composer_specification
                )
                chimpflow_composers_set_default(chimpflow_composer)
    ...
    async def _fetch_image(self, opaque, request_dict):
        ...
        if direction != 0:
            ...
            records = await chimpflow_datafaces_get_default().query(
    ...
    async def _fetch_image_list(self, opaque, request_dict):
        ...
        html = chimpflow_composers_get_default().compose_image_list(records)
        filters = {
    ...
    async def _set_target_position(self, opaque, request_dict):
        ...
        await chimpflow_datafaces_get_default().execute(sql, subs)
