...
from chimpflow_api.datafaces.constants import Commands, Keywords
...
# Types of chimpflow_dataface.
# Global chimpflow_dataface.
from chimpflow_lib.datafaces.datafaces import Datafaces

logger = logging.getLogger(__name__)

thing_type = "chimpflow_lib.chimpflow_datafaces.aiohttp"


# ------------------------------------------------------------------------------------------
class Aiohttp(Thing, BaseAiohttp):
    """
    Object implementing remote procedure calls for chimpflow_dataface methods.
    """
    ...
    def __init__(self, specification=None):
        ...
        self.__actual_chimpflow_dataface = None
    ...
    def activate_thread(self, loop):
        ...
        try:
            threading.current_thread().name = "chimpflow_dataface"
    ...
    async def activate_coro(self):
        ...
        try:
            ...
            # Build a local chimpflow_dataface for our back-end.
            self.__actual_chimpflow_dataface = Datafaces().build_object(
            ...
            await self.__actual_chimpflow_dataface.start()
    ...
    async def direct_shutdown(self):
        ...
        try:
            ...
            await self.__actual_chimpflow_dataface.disconnect()

        except Exception as exception:
            raise RuntimeError(
                callsign(
                    self,
                    explain(exception, "disconnecting local chimpflow_dataface"),
                )
            )
    ...
    async def __do_actually(self, function, args, kwargs):
        ...
        function = getattr(self.__actual_chimpflow_dataface, function)
