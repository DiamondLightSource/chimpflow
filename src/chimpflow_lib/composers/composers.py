...
def chimpflow_composers_set_default(composer):
    global __default_composer
    __default_composer = composer


def chimpflow_composers_get_default():
    global __default_composer
    if __default_composer is None:
        raise RuntimeError("chimpflow_composers_get_default instance is None")
    return __default_composer


def chimpflow_composers_has_default():
    global __default_composer
...
class Composers(Things):
    """
    List of available chimpflow_composers.
    """
    ...
    def lookup_class(self, class_type):
        ...
        if class_type == "chimpflow_lib.chimpflow_composers.html":
            from chimpflow_lib.composers.html import Html

            return Html

        elif class_type == "chimpflow_lib.chimpflow_composers.text":
            from chimpflow_lib.composers.text import Text
