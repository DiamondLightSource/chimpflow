...
from chimpflow_api.detectors.context import Context as DetectorContext

# Base class for an asyncio context.
from chimpflow_lib.contexts.base import Base as ContextBase

# Things created in the context.
from chimpflow_lib.detectors.detectors import Detectors, detectors_set_default

logger = logging.getLogger(__name__)


thing_type = "chimpflow_lib.detectors.context"


class Context(ContextBase):
    """
    Asyncio context for a detector object.
    On entering, it creates the object according to the specification (a dict).
    If specified, it starts the server as a coroutine, thread or process.
    If not a server, then it will instatiate a direct access to a detector.
    ...
    def __init__(self, specification: Dict):
        ...
        Args:
            specification (Dict): specification of the detector object to be constructed within the context.
                The only key in the specification that relates to the context is "start_as", which can be "coro", "thread", "process" or None.
                All other keys in the specification relate to creating the detector object.
        """
    ...
    async def aenter(self) -> None:
        ...
        Establishes the global (singleton-like) default detector.
        """

        # Build the object according to the specification.
        self.server = Detectors().build_object(self.specification())

        # If there is more than one detector, the last one defined will be the default.
        detectors_set_default(self.server)
        ...
        self.__api_context = DetectorContext(self.specification())
