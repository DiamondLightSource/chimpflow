...
class TestGoodConfigurationEnv:
    def test(self, constants, logging_setup, output_directory):
        ...
        # ---------------------------------------------------
        environ = {
            Envvar.CHIMPFLOW_CONFIGFILE: good_config_filename,
        }
...
class TestBadConfigurationEnv:
    def test(self, constants, logging_setup, output_directory):
        ...
        BadConfigurationEnvTester(
            "RuntimeError", "CHIMPFLOW_CONFIGFILE is not set"
        ).main(
            constants,
            {},
            output_directory,
        ...
        BadConfigurationEnvTester("RuntimeError", "does_not_exist.yaml").main(
            constants,
            {
                Envvar.CHIMPFLOW_CONFIGFILE: "does_not_exist.yaml",
            },
            output_directory,
        )
...
class GoodConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        chimpflow_configurator = Configurators().build_object(specification)

        # Perform symbol replacement.
        chimpflow_configurator.substitute({"output_directory": output_directory})

        # Remove top level key of this name.
        chimpflow_configurator.remove(["key_to_remove"])

        # Get the current dict representation of the configuration.
        current = await chimpflow_configurator.load()
        ...
        value = chimpflow_configurator.require("multi.level1.level2")
        assert value == "value_at_level2"

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = chimpflow_configurator.require("multi.level1.level3")

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = chimpflow_configurator.require("multi.level1.level2.level3")

        # Substitute in token.
        value = chimpflow_configurator.resolve(
...
class BadConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        try:
            ...
            await chimpflow_configurator.load()

            logger.info(describe("loaded yaml", chimpflow_configurator.get_current()))
        except Exception as exception:
